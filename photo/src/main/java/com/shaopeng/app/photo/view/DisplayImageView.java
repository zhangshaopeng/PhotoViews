...
import com.shaopeng.app.photo.R;
import com.shaopeng.app.photo.adapter.PhotoAdapter;
import com.shaopeng.app.photo.bean.PhotoBean;
import com.shaopeng.app.photo.bean.PhotoParameter;
import com.shaopeng.app.photo.tools.DensityTool;
import com.shaopeng.app.photo.tools.DownLoadCallBack;
import com.shaopeng.app.photo.tools.DownloadThread;
...
public class DisplayImageView extends RelativeLayout {
    ...
    private List<PhotoShowView> viewList;
    ...
    public DisplayImageView(Context context, @Nullable AttributeSet attrs, int defStyleAttr) {
        ...
        if (attrs != null) {
            TypedArray typedArray = context.obtainStyledAttributes(attrs, R.styleable.DisplayImageView);
            animDuration = typedArray.getInt(R.styleable.DisplayImageView_animDuration, DEFAULT_ANIM_DURATION);
            saveText = typedArray.getString(R.styleable.DisplayImageView_saveText);
            saveTextSize = typedArray.getDimensionPixelSize(R.styleable.DisplayImageView_saveTextSize, 0);
            saveTextColor = typedArray.getColor(R.styleable.DisplayImageView_saveTextColor, 0);
        }
        else {
     ...
     */
    public void showPhotoGallery(int index, List<PhotoBean> photoList, ImageView clickImageView) {
    ...
    private void setGalleryPhotoList(List<PhotoBean> list, PhotoParameter parameterModel) {
//        boolean isFocus = requestFocus();
    ...
    private void handleZoomAnimation() {
        // 屏幕的宽高
        this.mScreenRect = DensityTool.getDisplayPixes(getContext());
    ...
    private void initView() {
        View view = View.inflate(getContext(), R.layout.layout_view, this);
    ...
    PhotoShowView galleryPhoto = new PhotoShowView(getContext(), photoList.get(i));
        ...
        viewPager.setOnPageChangeListener(new ViewPager.OnPageChangeListener() {
            ...
            public void onPageSelected(int position) {
                DisplayImageView.this.position = position;
    ...
    private void saveBitmapToLocation() {
        DownloadThread download = new DownloadThread(getContext(), photoList.get(position).photoSource, new DownLoadCallBack() {
            @Override
    ...
    private void calculateScaleAndStartZoomInAnim(final PhotoParameter photoData) {
        ...
        int translationY = (photoData.locOnScreen[1] + photoData.imageHeight / 2) - (int) ((mScreenRect.height() + DensityTool.getStatusBarHeight(getContext())) / 2);
    ...
    private void calculateScaleAndStartZoomOutAnim() {
        ...
        int translationY = (firstClickItemParameterModel.locOnScreen[1] + firstClickItemParameterModel.imageHeight / 2) - (int) ((mScreenRect.height() + DensityTool.getStatusBarHeight(getContext())) / 2);
